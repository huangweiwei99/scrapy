        ...
        if response.status == 200:
            try:
                ...
                if str(meta_detail['product']['productId']) == item_id:
                    k_detail = self.crawler.stats.get_value('detail_{0}'.format(item_id))

                    if k_detail is None:
                        self.crawler.stats.set_value('detail_{0}'.format(item_id), meta_detail)

                    if len(re.findall(r'transactionOverview', url)) > 0:
                        # print('transactionOverview')
                        res_data = json.loads(response.text)
                        detail = self.crawler.stats.get_value('detail_{0}'.format(item_id))
                        detail['transaction_overview'] = res_data['data'] if res_data['success'] else []
                        self.crawler.stats.set_value('detail_{0}'.format(item_id), detail)
                        feature = 'transactionOverview'
                        # print('-------------------------------------')
                    ...
                    print('第 {0} 个'.format(self.crawler.stats.get_value('cnt')))
                    print('公司: {0}'.format(detail['seller']['companyName']))
                    print('产品ID {0} 功能: {1}'.format(item_id, feature))
                    print('产品: {0}'.format(detail['product']['subject']))
                    print('+++++++++++++++++++++++++++++++++++++++++++++===========================')
